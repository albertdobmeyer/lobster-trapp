...
async def test_simple_arithmetic(bot):
    reply = await bot.send_and_wait("what is 2+2? reply with just the number.", timeout=60)
    assert "4" in reply.text, f"Expected '4' in reply, got: {reply.text!r}"


async def test_no_blocked_events_on_benign_request(bot, proxy_log):
    """A benign message shouldn't trigger any BLOCKED/EXFIL_BLOCKED events."""
    await bot.send_and_wait("what day of the week was July 4, 1776?", timeout=60)
...
async def test_anthropic_200_chain(bot, proxy_log):
    """Verify the Anthropic call returns HTTP 200 (no auth/billing errors)."""
    await bot.send_and_wait("name one color", timeout=60)
